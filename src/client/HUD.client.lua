...
runLabel.Size = UDim2.new(1, -20, 0, 86)
runLabel.Position = UDim2.fromOffset(10, 48)
...
stashLabel.Size = UDim2.new(1, -20, 0, 86)
stashLabel.Position = UDim2.fromOffset(10, 134)
...
messageLabel.Position = UDim2.fromOffset(10, 220)
...
inventoryUpdate.OnClientEvent:Connect(function(snapshot)
	runLabel.Text = ("Raid Backpack ($%d):\n%s"):format(snapshot.runValue or 0, summarize(snapshot.runInventory))
	stashLabel.Text = ("Stash ($%d):\n%s\nExtracts: %d | Deaths: %d"):format(
		snapshot.stashValue or 0,
		summarize(snapshot.stash),
		snapshot.stats and snapshot.stats.extractions or 0,
		snapshot.stats and snapshot.stats.deaths or 0
	)
end)
